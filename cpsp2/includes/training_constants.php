...
function training_form_type_options(string $program = ''): array
{
    // Same Form Types for ALL programs (UROGYN, OBGYN, IMM, MCPS, FCPS-II)
    // Extracted directly from the live CPSP e-Portal
    return [
...
function training_form_type_label(string $id, string $program = ''): string
{
    $m = training_form_type_options($program);
...
/**
 * Build lookup id=>label from competency tree for the given program.
 *
 * @param string $program 'urogyn', 'obgyn', or '' for default IMM/FCPS-II tree
 * @return array<int,string>
 */
function training_competency_label_map(string $program = ''): array
{
    require_once __DIR__ . '/../data/competencies.php';
    $map = [];
    foreach (competency_tree_data($program) as $g) {
